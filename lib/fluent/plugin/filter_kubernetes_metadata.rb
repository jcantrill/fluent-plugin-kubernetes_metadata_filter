...
module Fluent
  class KubernetesMetadataFilter < Fluent::Filter
    ...
    def fetch_pod_metadata(namespace_name, pod_name)
      log.trace("fetching pod metadata: #{namespace_name}/#{pod_name}") if log.level == Fluent::Log::LEVEL_TRACE
      begin
        metadata = @client.get_pod(pod_name, namespace_name)
        unless metadata
          log.trace("no metadata returned for: #{namespace_name}/#{pod_name}") if log.level == Fluent::Log::LEVEL_TRACE
          @stats.bump(:pod_cache_api_nil_not_found)
        else
          begin
            log.trace("raw metadata for #{namespace_name}/#{pod_name}: #{metadata}") if log.level == Fluent::Log::LEVEL_TRACE
            metadata = parse_pod_metadata(metadata)
            @stats.bump(:pod_cache_api_updates)
            log.trace("parsed metadata for #{namespace_name}/#{pod_name}: #{metadata}") if log.level == Fluent::Log::LEVEL_TRACE
            return metadata
          rescue Exception=>e
            log.debug(e)
            @stats.bump(:pod_cache_api_nil_bad_resp_payload)
            log.trace("returning empty metadata for #{namespace_name}/#{pod_name} due to error") if log.level == Fluent::Log::LEVEL_TRACE
          end
        end
    ...
    def fetch_namespace_metadata(namespace_name)
      log.trace("fetching namespace metadata: #{namespace_name}") if log.level == Fluent::Log::LEVEL_TRACE
      begin
        metadata = @client.get_namespace(namespace_name)
        unless metadata
            log.trace("no metadata returned for: #{namespace_name}") if log.level == Fluent::Log::LEVEL_TRACE
            @stats.bump(:namespace_cache_api_nil_not_found)
        else
          begin
            log.trace("raw metadata for #{namespace_name}: #{metadata}") if log.level == Fluent::Log::LEVEL_TRACE
            metadata = parse_namespace_metadata(metadata)
            @stats.bump(:namespace_cache_api_updates)
            log.trace("parsed metadata for #{namespace_name}: #{metadata}") if log.level == Fluent::Log::LEVEL_TRACE
            return metadata
          rescue Exception => e
            log.debug(e)
            @stats.bump(:namespace_cache_api_nil_bad_resp_payload)
            log.trace("returning empty metadata for #{namespace_name} due to error") if log.level == Fluent::Log::LEVEL_TRACE
          end
        end
    ...
    def get_pod_metadata(key, namespace_name, pod_name, record_create_time)
        ...
        if ids == :orphaned
          log.trace("orphaning message for: #{namespace_name}/#{pod_name} ") if log.level == Fluent::Log::LEVEL_TRACE
          return {
